...
"""Simple Vehicles Routing Problem (VRP).
   ...
   The solver stop after improving solution 15 times or after 5 seconds.
...
def print_intermediate_solution(manager, routing):
    """Prints solution on console."""
    print('################')
    print(f'Solution Objective: {routing.CostVar().Value()}')
    total_distance = 0
    for vehicle_id in range(manager.GetNumberOfVehicles()):
        index = routing.Start(vehicle_id)
        plan_output = 'Route for vehicle {}:\n'.format(vehicle_id)
    ...
    class RoutingMonitor:
        ...
        def __call__(self):
            ...
            if not self.objectives or objective < self.objectives[-1]:
              ...
              print_intermediate_solution(self._routing_manager, self._routing_model)
              self._counter += 1
...
def main():
    ...
    if solution:
        print(f'Best objectives: {routing_monitor.objectives[-1]}')
    else:
